...
export const StartNotes = ({title,body = '',_id,pagina,id,arraysFiles}) => {
...
export const startUpdateEditFiles = ({_id,pagina,id}) => {
  return async(dispatch,getState) => {
    const { ItemSet } = await getState().actividades;
...
export const updateNotes = ({pagina,_id,id,noteId,title,body,arraysFiles}) => {
  return async(dispatch,getState) => {
    for (let i = 0; i < arraysFiles.length; i++) {
      const formData = new FormData();
      formData.append('file',arraysFiles[i]);
...
export const startUpdateFiles = ({file,arraysFiles}) => {
  return async(dispatch,getState) => {
    // const {file} = file;
    // const formData = new FormData();
