...
import { closeNewPlan, onActivePLan, onActiveSelect, onActualizarActiveList, onClearActividades, onCurrenItem, 
...
export const startSavingActivitie = ({ title,name }) => {
  return async (dispatch, getState) => {
    const { actividades,guardar,active } = await getState().actividades;
    const [{list}] = guardar;
  
    const { data } = await calidadiesApi.post('/events', { title, list:list,pogramas:name});
    const {evento} = data;
    const {_id} = evento;
     dispatch(startGetList(_id,name));
...
export const startGetList = (_id,name) => {
  return async (dispatch, getState) => {
...
export const startGuardado = ({name}) => {
  return async (dispatch, getState) => {
    console.log(name);
 
    const { data } = await calidadiesApi.get('/events')
    const {eventos} = data;
     const filteredActividades = eventos.filter(
       (active) => active.pogramas === name
     );
     console.log(filteredActividades);

    //  if(eventosFiltrados.length === 0) return;
    dispatch(onSavingNewPlan(filteredActividades))
  }
}
...
export const startDeleteItem = ({pagina,_id}) => {
  return async(dispatch) => {
    const {data} = await calidadiesApi.delete(`/events/item/${pagina}/${_id}`);
    const {eventoActualizado} = data;
    const {list} = eventoActualizado;
    dispatch(onActualizarActiveList());
    for(let i = 0; i < list.length; i++){
      const { listItem,_id } = list[i];
      const {titleItem} = listItem;
      dispatch(onSetLisItem(titleItem));
